...
class MessageServiceTest {
    ...
    void createMessage_NewUser_ShouldCreateConversationAndMessage() {
        ...
        assertThat(result.senderType()).isEqualTo(SenderType.USER.toString());
    ...
    void processAndRespondToMessage_Success_ShouldProcessCorrectly() {
        ...
        // messagingTemplate.convertAndSendToUser()가 성공(SUCCESS) 상태의
        // MessageResponse.Chat 객체와 함께 호출되는지 검증
        ...
        assertThat(capturedResponse.modelType()).isEqualTo(AIModelType.GPT.toString());
    ...
    void processAndRespondToMessage_AIModelFailure_ShouldSendErrorResponse() {
        ...
        // messagingTemplate.convertAndSendToUser()가 에러(ERROR) 상태의 MessageResponse.Chat
        // 객체와 함께 호출되는지 검증
        ArgumentCaptor<MessageResponse.Chat> responseCaptor = ArgumentCaptor.forClass(MessageResponse.Chat.class);
        verify(messagingTemplate).convertAndSendToUser(
                eq(userId.toString()),
